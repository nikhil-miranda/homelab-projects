...
## 6. Clone repo and stage `/opt/mediastack`

```bash
cd /opt
git clone https://github.com/nikhil-miranda/homelab-projects.git
ln -s /opt/homelab-projects/proxmox-mediastack /opt/mediastack
ln -s /opt/homelab-projects/.env /opt/mediastack/.env
cd /opt/mediastack
```

The first symlink keeps the conventional `/opt/mediastack` path while files live in the git working tree. `git pull` updates the running config in place.
...
```bash
cp /opt/homelab-projects/.env.example /opt/homelab-projects/.env
chmod 600 /opt/homelab-projects/.env
nano /opt/homelab-projects/.env
```
